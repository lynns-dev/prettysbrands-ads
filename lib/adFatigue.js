...
// A deliberate, one-off "duplicate this ad set" action triggered from the
// Cost-cap bidding table — unlike the automatic fatigue pass above, this
// isn't gated by the single-active-ad rule or the spend-vs-budget check:
// the operator has already looked at the numbers (e.g. an ad set running
// over its cost cap) and decided to refresh it themselves, multi-ad ad sets
// included. Deep-copying a multi-ad ad set duplicates every ad inside it,
// which is expected here since the whole ad set is what's being refreshed.
export async function duplicateAdSetManually(brand, adSetId, adSetName) {
  const base = { adSetId, adSetName, manual: true };
  try {
    const newAdSetId = await copyAdSet(adSetId, { deepCopy: true, statusOption: 'ACTIVE' });
    await updateAdSetStatus(adSetId, 'PAUSED');
    const applied = { ...base, action: 'refreshed', newAdSetId, appliedAt: new Date().toISOString() };
    await recordRefreshes(brand.id, [applied]);
    return applied;
  } catch (err) {
    const applied = { ...base, action: 'failed', reason: err.message, appliedAt: new Date().toISOString() };
    await recordRefreshes(brand.id, [applied]);
    throw err;
  }
}
