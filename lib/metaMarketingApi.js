...
// Day-by-day spend/revenue for a set of ids (time_increment=1), used by
// trend analysis to see how performance moved across a lookback window
// rather than just the window's total. Keyed by date, then by id.
export async function getInsightsTimeSeries(adAccountId, { level, ids, since, until }) {
  if (!ids || ids.length === 0) return {};
  const idKey = level === 'campaign' ? 'campaign_id' : level === 'ad' ? 'ad_id' : 'adset_id';
  const idField = level === 'campaign' ? 'campaign.id' : level === 'ad' ? 'ad.id' : 'adset.id';
...
// Meta's Ad Set Copies endpoint — used by the fatigue-refresh feature to
// give a stale ad a fresh ad ID (and, with deepCopy, a fresh ad set) while
// keeping the same targeting/budget/cost-cap. Returns the new ad set's id.
// deepCopy also copies every ad inside the source ad set, so this is only
// meaningful for the single-ad-per-ad-set "scaling" structure this app
// assumes for cost-cap ad sets.
export async function copyAdSet(adSetId, { deepCopy = true, statusOption = 'ACTIVE' } = {}) {
