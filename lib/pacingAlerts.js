...
export function computeDailyPacing({ dailyBudgetCents, spendTodayCents, now = new Date() }) {
  ...
  const pctOfDailyBudget = Math.round((spendTodayCents / dailyBudgetCents) * 1000) / 10;
  const hoursElapsed = now.getUTCHours() + now.getUTCMinutes() / 60;
  if (hoursElapsed < MIN_HOURS_ELAPSED) {
    return { dailyBudgetCents, spendTodayCents, pctOfDailyBudget, status: 'too_early', variancePct: 0, hoursElapsed, projectedEndOfDayCents: spendTodayCents };
  }
  ...
  return { dailyBudgetCents, spendTodayCents, pctOfDailyBudget, projectedEndOfDayCents, variancePct, status, hoursElapsed };
}
