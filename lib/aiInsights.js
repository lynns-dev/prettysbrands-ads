...
// zod/v4, not the top-level 'zod' classic export — zodOutputFormat's
// toJSONSchema() reads schema._zod.def, a shape only zod/v4-built schemas
// have; a classic-zod schema throws "Cannot read properties of undefined
// (reading 'def')" when passed to it.
...
export async function findWinningTestAds(brand) {
  ...
  // Scans both active and inactive/paused campaigns matching the pattern —
  // a paused test can still be worth promoting if it already proved itself
  // before it stopped running.
  const testingCampaigns = campaigns.filter((c) => c.name.toLowerCase().includes(pattern));
  if (testingCampaigns.length === 0) {
    return { ads: [], message: `No campaigns matched "${brand.testingCampaignPattern}".` };
  }
  ...
  const testingAds = ads.filter((a) => testingAdSetIds.has(a.adset_id));

  if (testingAds.length === 0) {
    return { ads: [], message: 'No ads found in the matched testing campaigns.' };
  }
...
Money figures in the data are in cents. The dataset includes both active and paused/inactive ads and campaigns (adStatus/campaignStatus) — a paused ad can still be a "winner" if it already proved itself before it stopped running; mention its current status in the reasoning when it's not active. Give a one or two sentence reasoning per ad that cites the actual spend/ROAS numbers, and classify every ad in the dataset — not just the winners.`,
    messages: [{
      role: 'user',
