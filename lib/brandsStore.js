...
const DEFAULTS = {
  ...
  // Required for creative uploads (lib/creativeUpload.js): every ad needs a
  // Facebook Page identity, and every purchase-optimized ad set needs a
  // pixel to attribute conversions to.
  pageId: null,
  pixelId: null,
};
...
export async function createBrand(input) {
  ...
  const brand = {
    ...
    pageId: input.pageId ? String(input.pageId).trim() : null,
    pixelId: input.pixelId ? String(input.pixelId).trim() : null,
    id: randomUUID(),
    createdAt: now,
    updatedAt: now,
  };
...
export async function updateBrand(id, patch) {
  ...
  if ('pageId' in patch) merged.pageId = patch.pageId ? String(patch.pageId).trim() : null;
  if ('pixelId' in patch) merged.pixelId = patch.pixelId ? String(patch.pixelId).trim() : null;
