...
// compares the whole current lookback period against the equal-length
// period right before it, so a "decline" badge means a sustained drop
// across the period, not one blip. +/-20% is the line between "this is
// just noise" and "this ad set is actually trending somewhere."
...
export async function previewBrandAdjustments(brand) {
  ...
  const prevRange = previousPeriodRange(brand.lookbackDays);
